BirdDesigner2 = ui.Frame:new()
Frame = ui.Frame

function BirdDesigner2:init()
	Frame.init(self)	
	...
	local itemSelectionFrame = BirdDesigner2SelectionFrame:new()
...
function BirdDesigner2:setupRecipeButtons()
	...
	if self.selectedItem ~= nil then
		...
		local recipeButton1 = self:getChild("recipeButton1")	
		recipeButton1:setImage(self.nestSprites[self.selectedItem.nestIndex])
		_G.table.insert(buttons, recipeButton1)
		
		local recipeButton2 = self:getChild("recipeButton2")	
		recipeButton2:setImage(self.eggSprites[self.selectedItem.eggIndex])
		_G.table.insert(buttons, recipeButton2)
		
		local recipeButton3 = self:getChild("recipeButton3")	
		recipeButton3:setImage(self.accessorySlot3Sprites[self.selectedItem.accessoryUpIndex])
		_G.table.insert(buttons, recipeButton3)
		
		local recipeButton4 = self:getChild("recipeButton4")	
		recipeButton4:setImage(self.accessorySlot3Sprites[self.selectedItem.accessoryDownIndex])
		_G.table.insert(buttons, recipeButton4)
		...
		for i = 1, 4 do 
			...
			local celPosX = startX + colIndex * cellMaxWidth + cellMaxWidth * 0.5
			local celPosY = startY + rowIndex * cellMaxHeight + cellMaxHeight * 0.5
			
			local scaleX = cellMaxWidth / recipeButton.w
			local scaleY = cellMaxHeight / recipeButton.h
...
function BirdDesigner2:setHatchery(hatchery)
...
function BirdDesigner2:openFrame(frame)
	self.currentFrame = frame
	frame.visible = true
	
end

function BirdDesigner2:closeCurrentFrame()
...
function BirdDesigner2:setSelection(nestIndex, eggIndex, accessoryUpIndex, accessoryDownIndex, objectId)
...
function BirdDesigner2:showUnderView()
	if self.selectedItem ~= nil then
		for i = 1, 4 do 
			local recipeButton = self:getChild("recipeButton" .. i)	
			recipeButton.visible = true
...
function BirdDesigner2:hideUnderView()
	for i = 1, 4 do 
		local recipeButton = self:getChild("recipeButton" .. i)	
		recipeButton.visible = false
	end
...
function BirdDesigner2:onPointerEvent(eventType,x,y)
...
function BirdDesigner2:update(dt, time) 
	...
	local isBody = self.selectedObject ~= nil and self:getIndexInTable(Bird.Sprites.Body, self.selectedObject.sprite) > 0
	...
	if gamelua.keyPressed["DELETE"] and self.selectedObject ~= nil and (self.selectedObject == self.accessoryUp or self.selectedObject == self.accessoryDown) then
...
function BirdDesigner2:checkObjectBounds(object)
...
function BirdDesigner2:getDistance(x1,y1,x2,y2)
	return _G.math.sqrt( (x2-x1) * (x2-x1) + (y2-y1) * (y2-y1))
end

function BirdDesigner2:normalize(x,y)
	local size = self:getDistance(x,y,0,0)
	return {x = x / size, y = y / size}
end

function BirdDesigner2:transformCoordinates(angle, x, y)
...
function BirdDesigner2:onEntry()	
...
function BirdDesigner2:setupBirdForEditing()
...
function BirdDesigner2:save()
	...
	if self.selectedItem ~= nil then
		...
		local basicBodySprite = Bird.Sprites.Body[self.prefabsTableToUse[self.selectedItem.objectId].shape]
		local basicBeakSprite = Bird.Sprites.Beaks[self.prefabsTableToUse[self.selectedItem.objectId].shape]
		local basicEyesSprite = Bird.Sprites.Eyes[self.prefabsTableToUse[self.selectedItem.objectId].shape]
...
function BirdDesigner2:getSpriteIndexInList(list, sprite)
	local index = 1
...
function BirdDesigner2:getIndexInTable(tableObj, element)
	local index = 1
...
function BirdDesigner2:draw(x, y)
	...
	for k, v in _G.pairs(self.prefabsTableToUse) do
		if #v.sprites == 3 then
			...
			if isDefault == true then
				...
				v.sprites = {	{sprite = Bird.Sprites.Body[v.shape], x = 0, y = 0, scale = 1, angle = 0},										
								{sprite = Bird.Sprites.Eyes[v.shape], x = 0, y = 0, scale = 1, angle = 0},
								{sprite = Bird.Sprites.Beaks[v.shape], x = 0, y = 0, scale = 1, angle = 0},
							}
			end
