...
function SliderFrame:init()
	...
	self.clip = {x1 = -self.px + self.x, y1 = -self.py + self.y, clipW = self.w, clipH = self.h} 

	local bgImage = Image:new()
	bgImage.name = "bgImage"
	_G.table.insert(self.children,bgImage)
	...
	content.x = 0
	content.y = 0		
	_G.table.insert(self.children,content)
...
function SliderFrame:onPointerEvent(eventType, x, y)
	...
	if not (x >= self.x - self.px and x <= self.x + (self.w - self.px) and y >= self.y - self.py and y <= self.y + (self.h - self.py)) then			
		self.state = "FREE"
		self.velocityY = 0
		self.dragStartX = nil
		self.dragStartY = nil
	...
	elseif eventType == "LRELEASE" then
		...
		elseif self.scrollDirection == "y" then		
			if self.dragStartY ~= nil and self.dragLastY ~= nil then
...
function SliderFrame:update(dt, time)
	...
	--print(_G.tostring(relativeScroll))
	--print(""..self.scrollY.."\n")
	...
	if self.velocityY ~= nil then
		...
		self.velocityY = self.velocityY * 0.80
	...
	if self.state == "FREE" then
		if relativeScroll > 1.0 then
			self.scrollY = self.scrollY + 100 * dt * _G.math.max(1.0, relativeScroll * 3)		
		elseif relativeScroll < 0 then
			self.scrollY = self.scrollY - 100 * dt * _G.math.max(1.0, _G.math.abs((relativeScroll - 1) * 3))	
		end
	end
...
function SliderFrame:draw(x, y)		
	--drawRect(1, 1, 0, 0.3, -self.px + x + self.x, 	-self.py + y + self.y,   -self.px + x + self.x + self.w,  -self.py + y + self.y + self.h, false) 
