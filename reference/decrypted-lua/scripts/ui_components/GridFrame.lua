...
---- GridFrame
-----------------------------------
---- Note: if this is slow and needs to be optimized, add all children to child - list also as normally
----       just take care of removing both instaces (this one keeps children only in self.grid)
----	   this also means that current implementation of GridFrame needs to re-implement the API of Frame.
----	   I have only used this as vertical grid, using this for other purposes might need small adjustments.
----	   
-----------------------------------
GridFrame = gamelua.ui.Frame:new()

function GridFrame:init()
	-- rows of grid, each row contains it's columns
	self.grid = {}	
	gamelua.ui.Frame.init(self)
	-- children are in grid.
	self.children = nil
...
function GridFrame:layout()
	...
	for i,row in _G.ipairs(self.grid) do
		local x = 0
		if(i > 1) then y = y + self:getMaxH(i - 1) end
...
--[[
function GridFrame:getContentBounds()
	local maxW = self:getGridWidth()
	local maxH = self:getGridHeight()
